...
def overlay_shirt(before, after):
    try:
        ...
        resized_img = ImageOps.fit(image, shirt.size, bleed=0.0, centering=(
            0.5, 0.5))  # resize input image to be the same size of shirt.png
        resized_img.paste(shirt, mask=shirt)  # overlay shirt.png over resized_img
        resized_img.save(after)  # save as output
